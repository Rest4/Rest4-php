...
class Template
{
    ...
    public static function getSafeOffset(&$scope,&$template)
    {
    ...
    // Replace includes
    public static function parseIncludes(&$scope,&$template)
    {
        $offset=-1;
    ...
    public static function parseConditions(&$scope,&$template)
    {
        $offset=-1;
    ...
    // Vars replacement
    public static function parseVars(&$scope,&$template)
    {
        $offset=-1;
        while (preg_match('/\{([a-z0-9_\.]+)\}/i', $template, $regs,PREG_OFFSET_CAPTURE)) {
            $offset=$regs[0][1];
            ...
            if ($thevar instanceof ArrayObject) {
                ...
                if ($thevar instanceof stdClass) {
                    $template = str_replace('{' . $varName . '}', '(stdClass)', $template);
                } else
                    if ($thevar||$thevar===0||$thevar==='0') {
    ...
    public static function parseLoops(&$scope,&$template, $bufferSize)
    {
        $offset=-1;
                    ...
                    while ($changes===true) {
                        ...
                        while (preg_match('/@'.$loopName.':([a-z0-9_\.|]+)@/Usi', $tItem, $itemregs)) {
                            ...
                            if ($itemregs[1]=='n') {
                                ...
                                if (($value3=Varstream::get($scope,$loopName.'.'.$key.'.'.$itemregs[1])) instanceof ArrayObject) {
                                    $tItem = str_replace('@' . $loopName . ':' . $itemregs[1] . '@', ''.$value3->count(), $tItem);
                                } else
                                    if ($value3||$value3==='0'||$value3===0) {
                            ...
                            for ($i=sizeof($conds)-1; $i>=0; $i--) {
                                ...
                                if ($conds[$i]=='1st') {
                                    ...
                                    if ($conds[$i]=='last') {
                                        ...
                                        if (ctype_digit($conds[$i])) {
                                            $thevar2=false;
                                            if ($itemN%$conds[$i]!==0) {
                                                $thevar2=true;
                                            }
                                        } else
                                            if (ctype_digit($conds[$i][0])&&preg_match('/^([0-9]+)on([0-9]+)$/', $conds[$i], $condMatches)) {
                                                $thevar2=false;
                                                if (($itemN+1)%$condMatches[2]==$condMatches[1]) {
                                                    $thevar2=true;
                                                }
                                            } else
                                                if (strpos($conds[$i],'laston')===0&&preg_match('/^laston([0-9]+)$/', $conds[$i], $condMatches)) {
                                                    $thevar2=false;
