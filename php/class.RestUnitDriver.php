    ...
    if ($tests->code!=RestCodes::HTTP_200) {
      ...
      foreach ($tests->vars->files as $file) {
        if((!(isset($file->isDir)&&$file->isDir))&&($this->queryParams->filter===''
          ...
          $test=new RestResource(new RestRequest(RestMethods::GET,
            '/'.($this->queryParams->multiple=='yes'?'mp':'')
            .'fs/tests/'.$file->name
            .($this->queryParams->multiple=='yes'?'?mode=merge':''),
            array('X-Rest-Local-Cache'=>'disabled')));
          $test=$test->getResponse();

          if ($test->code!=RestCodes::HTTP_200) {
            throw new RestException(RestCodes::HTTP_500,
              'Can\'t access the test: '.$file->name);
          } else {
            Varstream::import($testContent=new stdClass(),$test->getContents());
            $entry=new stdClass();
            $req=new RestRequest(
              RestMethods::getMethodFromString($testContent->request->method),
            ...
            $entry->result=$testContent->request->method.' '
              .$testContent->request->uri
              .' : '.$res->code.' '
              .constant('RestCodes::HTTP_'.$res->code.'_MSG');
            $entry->errors=new MergeArrayObject();
            if ($res->code!=$testContent->response->code) {
              $entry->errors->append(
                'Unexpected result: HTTP response code is '.$res->code
                .', '.$testContent->response->code.' expected.');
              $passed=false;
            }
            if (isset($testContent->response->headers)) {
              foreach ($testContent->response->headers as $header) {
                if ($res->getHeader($header->name)
                  &&$res->getHeader($header->name)!=$header->value) {
                  $entry->errors->append(
                    'Unexpected result: HTTP response header '.$header->name
                    .' value is "'.$res->getHeader($header->name)
            ...
            if (isset($res->vars)) {
              ...
              if(isset($testContent->response->content)&&
                  !Varstream::equals($res->vars,
                    $testContent->response->content)) {
                $entry->errors->append('Unexpected result: '
                  . 'HTTP response content differs.');
                $entry->errors->append(var_export($res->vars,true).'!='
                  .var_export($testContent->response->content,true));
                $passed=false;
              }
              // if error, showing content
              if ($this->queryParams->showcontent=='yes'
                ||$entry->errors->count()) {
            ...
            if ($this->queryParams->verbose=='yes'
              ||$entry->errors->count()) {
