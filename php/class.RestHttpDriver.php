...
class RestHttpDriver extends RestDriver
{
  ...
  private function prepare()
  {
    ...
    if ($this->queryParams->auth) {
      ...
      curl_setopt($this->_c, CURLOPT_USERPWD,
        $this->queryParams->user.':'.$this->queryParams->password);
    }

    $this->_c_headers=array();
    foreach ($this->request->headers as $name => $value) {
      if ($name=='User-Agent'||$name=='Accept'||$name=='Accept-Language'
        ||$name=='Accept-Charset') {
  ...
  private function exec()
  {
    ...
    $response->setHeader('Content-Type',
      curl_getinfo($this->_c,CURLINFO_CONTENT_TYPE));
    $response->setHeader('Content-Length',
      curl_getinfo($this->_c,CURLINFO_CONTENT_LENGTH_DOWNLOAD));
    ...
    curl_close($this->_c);
    if ($errorno) {
      throw new RestException(RestCodes::HTTP_500,'cURL got an error.',
        'Error '.$errorno.': '.$error.', uri: '.$this->_uri);
    }
  ...
  public function head()
  {
    ...
    curl_setopt($this->_c,CURLOPT_CUSTOMREQUEST,
      RestMethods::getStringFromMethod(RestMethods::HEAD));
  ...
  public function post()
  {
    ...
    if(($this->request->getHeader('Content-Type')=='text/varstream'
        ...
        &&($this->request->content instanceof ArrayObject
