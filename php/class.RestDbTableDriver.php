        ...
        if ((!isset($entry))||$entry->name!=$row['columnName']) {
          ...
          if (strpos($row['colType'], 'enum')===0) {
            ...
            if (strpos($row['colType'], 'set')===0) {
              ...
              if (strpos($row['colType'], 'decimal')===0) {
                $entry->input = 'input';
                $entry->type = 'number';
                $entry->filter = 'float';
                ...
                if(strpos($row['colType'], 'float')===0
                  ...
                  if (strpos($row['colType'], 'float')===0) {
                    ...
                    if (strpos($row['colType'], 'double')===0) {
                      $entry->precision=8;
                    }
                  if (strpos($row['colType'],'unsigned')!==false) {
                    $entry->min=0;
                  ...
                  if (strpos($entry->name,'lat')===0||$row['comment']=='lat') {
                    ...
                    if (strpos($entry->name,'lng')===0||$row['comment']=='lng') {
                      $hasLng=true;
                    }
                } else
                  if(strpos($row['colType'], 'tinyint')===0
                    ...
                    if (strpos($row['colType'], 'tinyint')===0) {
                      ...
                      if (strpos($row['colType'], 'tinyint')===0) {
                        ...
                        if (strpos($row['colType'], 'smallint')===0) {
                          if (strpos($row['colType'],'unsigned')!==false) {
                            $entry->min=0;
                            $entry->max=65535;
                          } else {
                          ...
                          if (strpos($row['colType'], 'mediumint')===0) {
                            if (strpos($row['colType'],'unsigned')!==false) {
                              $entry->min=0;
                              $entry->max=16777215;
                            } else {
                            ...
                            if (strpos($row['colType'], 'int')===0) {
                              ...
                              if (strpos($row['colType'], 'bigint')===0) {
                                if (strpos($row['colType'],'unsigned')!==false) {
                                  $entry->min=0;
                    ...
                    if(strpos($row['colType'], 'date')===0
                      ...
                      if(strpos($row['colType'], 'char')===0
                        ...
                        if ($entry->name=='password'||$row['comment']=='iparameter') {
                          ...
                          if ($entry->name=='name'||$entry->name=='lang'
                              ||$row['comment']=='parameter'||$row['comment']=='name') {
                            $entry->filter = 'parameter';
                            $entry->pattern = '[a-z0-9_]+';
                          } else
                            if(strpos($entry->name,'mail')===0
                                                             ||strpos($entry->name,'email')===0||$row['comment']=='mail') {
                              $entry->type='email';
                              $entry->filter = 'mail';
                            } else
                              if (strpos($entry->name,'url')===0||$row['comment']=='httpuri') {
                                $entry->filter = 'httpuri';
                              } else
                                if (strpos($entry->name,'uri')===0||$row['comment']=='uri') {
                                  $entry->filter = 'uri';
                                } else
                                  if(strpos($entry->name,'phone')===0
                        ...
                        if ($row['colType']=='tinyblob'||$row['colType']=='tinytext'
                          ...
                          if ($row['comment']) {
                            ...
                            if ($row['comment']=='xhtml') {
                              ...
                              if ($row['comment']=='xhtmlnb') {
                                $entry->filter = 'nbpcdata';
                              } else
                                if ($row['comment']=='xbbcode') {
                                  $entry->filter = 'bbpcdata';
                                } else
                                  if ($row['comment']=='xbbcodenb') {
                          ...
                          if ($row['colType']=='tinyblob'||$row['colType']=='tinytext') {
                            ...
                            if ($row['colType']=='blob'||$row['colType']=='text') {
                              $entry->max=65535;
                            } else
                              if ($row['colType']=='mediumblob'||$row['colType']=='mediumtext') {
                                $entry->max=16777215;
                              } else
                                if ($row['colType']=='longblob'||$row['colType']=='longtext') {
                                  $entry->max=4294967295;
                                }
                        }
          // Adding flags
