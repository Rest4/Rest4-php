...
class RestDbTableDriver extends RestVarsDriver
{
  ...
  public function head()
  {
    ...
    $this->core->db->query(
      ...
      .'  cols.ORDINAL_POSITION AS columnPosition,'
      .'	cols.COLUMN_DEFAULT AS defaultValue, cols.IS_NULLABLE as isNull,'
      .'  cols.DATA_TYPE AS dataType, cols.CHARACTER_MAXIMUM_LENGTH AS max,'
      .'  cols.CHARACTER_OCTET_LENGTH as min,'
      .'  cols.NUMERIC_PRECISION AS numericPrecision,'
      .'  cols.NUMERIC_SCALE AS scale, cols.COLUMN_TYPE AS colType,'
      .'  cols.COLUMN_KEY AS columnKey, cols.EXTRA AS extra,'
      .'	cols.COLUMN_COMMENT AS comment,'
      .'	refs.TABLE_NAME AS referredTable, refs.COLUMN_NAME AS referredColumn,'
      .'	cRefs.UPDATE_RULE AS referredUpdRule,'
      .'  cRefs.DELETE_RULE AS referredDelRule,'
      .'	links.REFERENCED_TABLE_NAME AS linkedTable,'
      .'	links.REFERENCED_COLUMN_NAME AS linkedColumn,'
      .'	cLinks.UPDATE_RULE AS linkedUpdRule,'
      .'  cLinks.DELETE_RULE AS linkedDelRule,'
      .'  joins.TABLE_NAME AS joinedTable,'
      .'	joins.COLUMN_NAME AS joinedColumn,'
      .'  cJoins.UPDATE_RULE AS joinedUpdRule,'
      .'  cJoins.DELETE_RULE AS joinedDelRule'
        ...
        if((!isset($entry))||$entry->name!=$row['columnName']) {
          ...
          if(strpos($row['colType'], 'enum')===0) {
            ...
            if($row['colType']=='datetime'||$row['colType']=='timestamp') {
              ...
              if($row['comment']=='day') {
                $entry->min='1900-01-01';
                $entry->max='1900-12-31';
                $entry->type='date';
                $entry->filter='day';
              } else {
            ...
            if($entry->name=='password'||$row['comment']=='iparameter') {
              ...
              ||$row['comment']=='parameter'
              ||$row['comment']=='name') {
              $entry->filter = 'parameter';
              $entry->pattern = '[a-z0-9_]+';
            } else if(strpos($entry->name,'mail')===0
              ||strpos($entry->name,'email')===0
              ||$row['comment']=='mail') {
              $entry->type='email';
              $entry->filter = 'mail';
            } else if(strpos($entry->name,'url')===0
              ||$row['comment']=='httpuri') {
              $entry->filter = 'httpuri';
            } else if(strpos($entry->name,'uri')===0
              ||$row['comment']=='uri') {
              $entry->filter = 'uri';
            } else if(strpos($entry->name,'phone')===0
              ||strpos($entry->name,'fax')===0
              ||strpos($entry->name,'gsm')===0
              ||$row['comment']=='phone') {
              $entry->type = 'tel';
              $entry->filter = 'phone';
              $entry->pattern = '\+[0-9]{2,3}\.[0-9]+';
            }
          } else if($row['colType']=='tinyblob'
            ||$row['colType']=='tinytext'
            ||$row['colType']=='blob'
            ||$row['colType']=='text'
            ||$row['colType']=='mediumblob'
            ||$row['colType']=='mediumtext'
            ||$row['colType']=='longblob'
            ||$row['colType']=='longtext') {
            ...
            if($row['colType']=='tinyblob'
              ||$row['colType']=='tinytext') {
              $entry->max=255;
            } else if($row['colType']=='blob'
              ||$row['colType']=='text') {
              $entry->max=65535;
            } else if($row['colType']=='mediumblob'
              ||$row['colType']=='mediumtext') {
              $entry->max=16777215;
            } else if($row['colType']=='longblob'
              ||$row['colType']=='longtext') {
              $entry->max=4294967295;
            }
          }
  ...
  public function post()
  {
    ...
    foreach ($schema->table->constraintFields as $field) {
      ...
      } else if(isset($field->references)) {
