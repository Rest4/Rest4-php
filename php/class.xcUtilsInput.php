...
class xcUtilsInput
{
  ...
  public static function filterValue($value, $type='text', $filter='parameter')
  {
    ...
    case 'number':
      ...
      case 'float':
        ...
        break;
      default:
        throw new Exception('self -> filterValue : the filter "'.$filter.'"'
          . ' is not supported for this type : '.$type.'.');
      }
      break;
    case 'date':
      switch ($filter) {
    ...
    case 'file':
      ...
      break;
    default:
      throw new Exception('self -> filterValue : the type "'.$type.'"'
        . ' is not supported.');
    }
  ...
  public static function filterAsCdata($string)
  {
    ...
    $string=html_entity_decode($string,
      ENT_QUOTES|(defined('ENT_HTML401')?constant('ENT_HTML401'):0),'UTF-8');
  ...
  {
    $string=trim($string);
    $string=preg_replace('/[' . utf8_encode('ÃÁÄÂÀ') . ']/u', 'A', $string);
    $string=preg_replace('/[' . utf8_encode('áäâàãå') . ']/u', 'a', $string);
    $string=preg_replace('/[' . utf8_encode('Ç') . ']/u', 'C', $string);
    $string=preg_replace('/[' . utf8_encode('ç') . ']/u', 'c', $string);
    $string=preg_replace('/[' . utf8_encode('Ð') . ']/u', 'D', $string);
    $string=preg_replace('/[' . utf8_encode('ð') . ']/u', 'd', $string);
    $string=preg_replace('/[' . utf8_encode('ÉÈËÊ') . ']/u', 'E', $string);
    $string=preg_replace('/[' . utf8_encode('éèëê') . ']/u', 'e', $string);
    $string=preg_replace('/[' . utf8_encode('ÏÎÌÍ') . ']/u', 'I', $string);
    $string=preg_replace('/[' . utf8_encode('ïîìí') . ']/u', 'i', $string);
    $string=preg_replace('/[' . utf8_encode('Ñ') . ']/u', 'N', $string);
    $string=preg_replace('/[' . utf8_encode('ñ') . ']/u', 'n', $string);
    $string=preg_replace('/[' . utf8_encode('ÕÖÔÒÓØ') . ']/u', 'O', $string);
    $string=preg_replace('/[' . utf8_encode('öôòóõø') . ']/u', 'o', $string);
    $string=preg_replace('/[' . utf8_encode('ÜÛÙÚ') . ']/u', 'U', $string);
    $string=preg_replace('/[' . utf8_encode('üûùú') . ']/u', 'u', $string);
    $string=preg_replace('/[' . utf8_encode('Ý') . ']/u', 'Y', $string);
    $string=preg_replace('/[' . utf8_encode('ÿý') . ']/u', 'y', $string);
    $string=preg_replace('/[' . utf8_encode('Æ') . ']/u', 'AE', $string);
    $string=preg_replace('/[' . utf8_encode('æ') . ']/u', 'ae', $string);
    $string=preg_replace('/[' . utf8_encode('') . ']/u', 'OE', $string);
    $string=preg_replace('/[' . utf8_encode('') . ']/u', 'oe', $string);
    $string=preg_replace('/[' . utf8_encode('ß') . ']/u', 'ss', $string);
  ...
  {
    if(preg_match('/^([0-9]{2,4})\-([0-9]{1,2})\-([0-9]{1,2})(( |T)([0-9]{1,2})'
      .'\:([0-9]{1,2})\:([0-9]{1,2}))?$/',$string,$matches)
      &&checkdate($matches[2], $matches[3], $matches[1])
  ...
  {
    if (preg_match('/^([0-9]+)\:([0-9]{1,2})(\:([0-9]{1,2}))?$/',
      $string,$matches)) {
      return true;
    }
  ...
  public static function filterAsRelativeuri($string)
  {
    if (self::isRelativeuri($string)) {
      // Replace generic data (http:// and / at the end)
      $string = preg_replace('/^(http:\/\/)(.+)([\/]?)$/','$2',$string);
       //rawurlencode(utf8_decode()) urlencode(utf8_decode())
       // || rawurlencode(utf8_decode())
       // + problem with already converted entities ?
       //-> http://www.php.net/manual/fr/function.urlencode.php#73366
      if (substr($string,strlen($string)-1)=='/') {
        $string=substr($string,0,strlen($string)-1);
      }
  ...
  public static function isUri($string)
  {
    ...
    // Domains accept special chars like é à etc... find the entire list and find a way to comply with it !
    if (preg_match('/^(([a-z0-9\.]+):\/\/(([a-z0-9]+)(:([a-z0-9]+))?@)?'
      . '([^ _\/\?#]*)(:([0-9]+))?\/)?([^\?#]*)(\?([^#]*))?(#(.*))?$/',$string)) {
      return true;
    }
  ...
  public static function filterAsInt($string)
  {
    if (self::isInt($string)) {
      return intval(preg_replace('/^(\-?)(\+?)([0-9]+)([,\.]?)([0-9]*)$/',
        '$1$3', $string));
    } else {
      return'';
    }
  ...
  public static function filterAsFloat($string)
  {
    if (self::isFloat($string)) {
      $string=preg_replace('/^(\-?)(\+?)([0-9]+)([,\.]?)([0-9]*)$/', '$1$3.$5',
        $string);
      if (substr($string,strlen($string)-1)=='.') {
        $string=substr($string,0,strlen($string)-1);
      }
  ...
  {
    if (preg_match('/^data:(([a-z]+)\/([a-z]+))?;base64,([a-z0-9\+\=\/]*)$/i',
      $string)) {
      return true;
    }
