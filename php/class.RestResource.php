...
class RestResource
{
  ...
  public function getResponse()
  {
    try {
      ...
      $resname = $this->request->controller
      ...
      if($this->request->method==RestMethods::GET
        &&isset(self::$_loadedResources[$resname])) {
        $this->response=self::$_loadedResources[$resname];
      }
      ...
      if($this->core->cache->type&&'none'!=$this->core->cache->type
        ...
        $res=new RestResource(new RestRequest(RestMethods::GET,
          '/cache/' . $this->core->cache->type . '/' . $resname));
        $res=$res->getResponse();
      ...
      if (!$this->response) {
        ...
        if($this->core->cache->type&&'none'!=$this->core->cache->type
          ...
          self::$_loadedResources[$resname]=$this->response;
            ...
            foreach ($uncache as $unc) {
              if ($unc) {
                $res=new RestResource(new RestRequest(RestMethods::POST,
                  '/cache/'.$this->core->cache->type.$unc.'callback.txt',
                  array(),'/'.$resname."\n"));
                $res->getResponse();
              }
            }
        ...
        if($this->core->cache->type&&'none'!=$this->core->cache->type
          ...
          array_push($uncache,'/'.$this->request->controller
            .($this->request->filePath ? substr($this->request->filePath, 0,
              strlen($this->request->filePath)-1) : ''));
          foreach ($uncache as $unc) {
