...
class RestXgpsCleanupDriver extends RestVarsDriver
{
  ...
  public function get()
  {
    ...
    foreach ($res->vars->files as $file) {
      if (strpos($file->name,'x1-')===0) {
        ...
        if (mktime(0, 0, 0, $date[4].$date[5], $date[6].$date[7],
          $date[0].$date[1].$date[2].$date[3])
          <time()-($this->queryParams->old*24*60*60)) {
          $entry=new stdClass();
          $entry->name=$file->name;
          $entry->date=$date[0].$date[1].$date[2].$date[3].'-'.$date[4].$date[5]
            .'-'.$date[6].$date[7];
  ...
  public function post() { // Should use delete method instead
    $response=$this->get();
