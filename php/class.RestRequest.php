...
class RestRequest extends RestMessage
{
  ...
  public function __construct($method, $uri='/', $headers=array(), $content='')
  {
    if (!$method) {
  ...
  public function getHeader($name,$type='text',$filter='iparameter')
  {
    $value=parent::getHeader($name);
    if ($value!==''&&!xcUtilsInput::filterValue($value,$type,$filter)) {
      throw new RestException(RestCodes::HTTP_400,
        'Header '.$name.' value do not match the right type'
        . ' ('.$type.','.$filter.')');
    }
  ...
  public function parseUri()
  {
    ...
    for ($i=0; $i<strlen($reluri); $i++) {
      if (($reluri[$i]=='.'&&strrpos($reluri,'.',(strpos($reluri,'?')===false?
        0:strpos($reluri,'?')-strlen($reluri)))===$i)||$reluri[$i]=='?') {
        break;
      }
      if ($reluri[$i]=='/') {
        if ($curNode=='') {
          throw new RestException(RestCodes::HTTP_400,
            'A uri node cannot be empty !'
            . ' (Node:'.($this->uriNodes->count()+1).')');
        }
    ...
    if (isset($reluri[$i])&&$reluri[$i]=='.') {
      ...
      // Should be Parameter, have to change when everything is ok
      if (!xcUtilsInput::isIParameter($this->fileExt)) {
        throw new RestException(RestCodes::HTTP_400,
          'Illegal character(s) found in the file extension (a-z/0-9 only)'
          . ' ['.$this->fileExt.']');
      }
    }
    ...
    if (isset($this->uriNodes[0])&&$this->uriNodes[0]) {
      ...
      if (!xcUtilsInput::isParameter($this->controller)) {
        throw new RestException(RestCodes::HTTP_400,
          'Illegal character(s) found in the controller name (a-z/0-9 only)');
      }
    } else {
      $this->controller='';
    }
    ...
    if (strpos($reluri,'?')!==false) {
      ...
      if ($this->queryString=='') {
        throw new RestException(RestCodes::HTTP_400,
          'Found "?" character but nothing after (uri: '.$reluri.').');
      }
    } else {
      $this->queryString='';
    }
  ...
  public function parseVarsContent()
  {
    $content=$this->content;
    $this->content=new stdClass();
    Varstream::import($this->content,$content);
  }
  public function parseBase64Content()
  {
  ...
  public function parseJsonContent()
  {
  ...
  public function parseFormUrlEncoded($string)
  {
    ...
    for ($i=0; $i<strlen($string); $i++) {
      if ($param->value==='') {
        if ($string[$i]=='&') {
          throw new RestException(RestCodes::HTTP_400,
            'Unterminated query string param ('.$param->name.')');
        }
        if ($string[$i]=='=') {
          if ($param->name=='') {
            throw new RestException(RestCodes::HTTP_400,
        ...
        if ($string[$i]=='&') {
          ...
          if ($string[$i]=='=') {
            throw new RestException(RestCodes::HTTP_400,
              'Can\'t put "=" char in a param value, please encode him'
              . ' (found in "'.$param->name.'" value: "'.$param->value.'").');
          }
    ...
    if ($i>0&&$string[$i-1]=='&') {
      throw new RestException(RestCodes::HTTP_400,
        'Orphelin "&" found at the query string end.');
    }
    if ($param->name) {
      $param->value=urldecode($param->value);
      if ($param->value==='') {
  ...
  public function sortAcceptHeader($name)
  {
  ...
  public function testAcceptHeader($name,$value)
  {
  ...
  public function leveledTestAcceptHeader($name,$value,$level=0)
  {
    $name=str_replace(' ', '-', ucwords(strtolower(str_replace('-',' ',$name))));
    $this->sortAcceptHeader($name);
    foreach ($this->_acceptHeaders[$name] as $key => $mask) {
