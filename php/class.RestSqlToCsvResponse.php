...
class RestSqlToCsvResponse extends RestStreamedResponse
{
  ...
  public function __construct($code=RestCodes::HTTP_200, $headers=array(),
    $request, $labels,$filesPath='',$downloadFilename='')
  {
    ...
    if (!$this->core->db->numRows()) {
      throw new RestException(RestCodes::HTTP_410,
        'No datas available with those criterias.');
    }
  }
      ...
      if ($row=$this->core->db->fetchArray($this->query)) {
        foreach ($this->labels as $label) {
          if (!isset($row[$label])) {
            ...
            $line.='"'.str_replace('"','\"',
              html_entity_decode($row[$label],ENT_QUOTES,'UTF-8')).'"'.';';  //|ENT_HTML401
          }
        }
        if ($this->filesPath&&isset($row['id'])) {
          $res=new RestResource(new RestRequest(RestMethods::GET,
            '/fsi/db/'.$this->filesPath.'/'.$row['id'].'/files.dat?mode=light'));
          $res=$res->getResponse();
          $vars=$res->vars;
          $line.=($res->code==RestCodes::HTTP_200&&isset($vars->files)?
            $vars->files->count():'0').';';
        }
        $line.="\r\n";
      }
