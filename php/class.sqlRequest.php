...
class sqlRequest
	...
	function setFields()
		...
		foreach(func_get_args() as $field)
			{
			if(($pos=strpos($field,'.'))>0&&!in_array(substr($field,0,$pos),$this->tables))
	...
	function setJoinClause($table, $clauses, $merge='')
		{
	...
	function getRequest()
		...
		if($join_size>0)
			...
			for($i=0; $i<$join_size; $i++)
				{
				$request.=' '.($this->join_clauses[$i]['merge']?$this->join_clauses[$i]['merge'].' ':'').sqlRequest::JOIN;
				$request.=' '.$this->join_clauses[$i]['table'].' '.sqlRequest::OPERATOR_ON;
		...
		if($where_size>0)
			...
			for($i=0; $i<$where_size; $i++)
				{
				$request.=' '.$this->where_clauses[$i].($i<$where_size-1?' '.sqlRequest::OPERATOR_AND :'');
				}
			}
		...
		if($this->limit)
			{
			$request.=' '.sqlRequest::LIMIT . ' ' . ($this->start?$this->start.', ':'') . $this->limit;
			}
		return $request;
		}
