...
class RestTemplatedResponse extends RestStreamedResponse
{
  ...
  private $scope;
  private $includeName;
  public function __construct($code, $headers, $template, $scope)
  {
  ...
  public function pump()
  {
    ...
    while ($curOffset>=0&&($this->bufferSize===0
      ||strlen($chunk)<=$this->bufferSize)) {
      // Parsing template instructions
      $curOffset=Template::parseConditions($this->core,$this->template);
      if(($newOffset=Template::parseLoops($this->core,$this->template,
        $this->bufferSize))>=0
