...
class RestDbEntryDriver extends RestVarsDriver
{
  ...
  public static function getDrvInf($methods=0)
  {
    $drvInf=parent::getDrvInf(RestMethods::GET|RestMethods::PUT
      |RestMethods::PATCH|RestMethods::DELETE);
  ...
  public function head()
  {
    ...
    $this->core->db->query('SELECT id FROM ' . $this->request->table
      .' WHERE id="'.$this->request->entry.'"');
  ...
  public function put()
  {
    ...
    // otherwise, we create one
      ...
      if($schema->table->nameField=='id') {
        throw new RestException(RestCodes::HTTP_400,
          'The given table has no name field.');
      }
      foreach ($schema->table->fields as $field) {
  ...
  public function delete()
  {
    ...
    // Deleting the entry
    $this->core->db->selectDb($this->request->database);
    $this->core->db->query('DELETE FROM ' . $this->request->table
      .' WHERE id="'.$this->request->entry.'"');
    $res=new RestResource(new RestRequest(RestMethods::DELETE,
      '/fs/db/'.$this->request->database.'/'.$this->request->table.'/'
      .$this->request->entry.'/?recursive=yes'));
    $res=$res->getResponse();
    // Setting cache directives
    $uncache='/db/'.$this->request->database.'/'.$this->request->table.'/'
      .'|/fs/db/'.$this->request->database.'/'.$this->request->table.'/';
