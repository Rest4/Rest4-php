...
class xcUtilsOutput
{
  ...
  public static function localizeAmount($number, $decimals, $currency, $format,
    $decpoint='',$thsep='')
  {
    return str_ireplace('C', $currency, str_ireplace('X',
      number_format($number, $decimals, $decpoint, $thsep), $format));
  }

  public static function localizeDate($date,$format,$days=null,$months=null)
  {
    preg_match('/^([0-9]{2,4})\-([0-9]{1,2})\-([0-9]{1,2})'
      . '( ([0-9]{1,2})\:([0-9]{1,2})\:([0-9]{1,2}))?$/',$date,$matches);
    $date=date($format,mktime((isset($matches[5])?$matches[5]:0),
  ...
  public static function localizePhoneNumber($number,$iformat,$lformat,
    $lindic,$nformat)
  {
    if ($number) {
      $lnumber='';
    ...
    $secure=false)
  {
    ...
    $string = str_replace(']]', '&#93;', $string);
    $tags = array('span', 'kbd', 'var', 'del', 'ins', 'div', 'strong', 'em',
      'dfn', 'cite', 'q', 'blockquote', 'p', 'br', 'a', 'ol', 'ul', 'li',
      'abbr', 'acronym', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'hr', 'pre',
      'address', 'img', 'tr', 'th', 'td', 'table', 'caption', 'thead',
      'tfoot', 'tbody', 'dl', 'dd', 'dt', 'map', 'area', 'code', 'samp',
      'sub', 'sup');
    if(!$secure) {
      array_push($tags, 'script', 'object', 'param');
    }
    $attributes = array('class', 'id', 'name', 'dir', 'title', 'lang', 'style',
      'href', 'hreflang', 'rel', 'rev', 'tabindex', 'type', 'accesskey',
      'charset', 'datetime', 'cite', 'alt', 'longdesc', 'usemap', 'src',
      'coords', 'shape', 'nohref', 'summary', 'scope');
    if(!$secure) {
      array_push($attributes, 'onclick', 'ondblclick', 'onkeydown', 'onkeypress',
        'onkeyup', 'onload', 'onmousedown', 'onmousemove', 'onmouseout',
        'onmouseover', 'onmouseup', 'onunload', 'onblur', 'onfocus', 'defer',
        'value', 'data', 'width', 'height');
    }
        ...
        if ($string[$i+1]=='/') {
          ...
          if ($curTag&&($all||in_array($curTag,$tags))) {
            ...
            if ($i+1<strlen($string)&&$string[$i]=='/'&&$string[$i+1]==']') {
              $opClTag=true;
            } else if ($i+2<strlen($string)&&$string[$i]==' '
              &&$string[$i+1]=='/'&&$string[$i+2]==']') {
              $opClTag=true;
            } else if ($i+1<strlen($string)&&$string[$i]==' '
              &&strpos($attributeChars,$string[$i+1])!==false) {
              while ($string[$i]==' ') {
                $curAttBegin=$i+1;
                // Attribute name loop
                for ($i=$i+1; $i<strlen($string); $i++) {
                ...
                if ($string[$i]=='=') {
                  for ($i=$i+1; $i<strlen($string); $i++) {
                    if ($string[$i]==']') {
                      ...
                      &&$string[$i+1]==']'&&$curAtt!='href'
                      &&$curAtt!='src'&&$curAtt!='cite') {
                      $opClTag=true;
                      break;
                    } else if ($i<strlen($string)&&$string[$i]==' '
                      &&$string[$i+1]=='/'&&$string[$i+2]==']') {
