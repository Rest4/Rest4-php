<?php
class RestSitePrivateDefaultDriver extends RestSitePrivateDriver
{
  static $drvInf;
  public static function getDrvInf($method=0)
  {
    $drvInf=new stdClass();
    $drvInf->name='Site: Default Private Driver';
  ...
  public function get()
  {
    ...
    $node=strtolower($this->request->uriNodes[4]);
    // Redirect if the fourth uri node equals to index
    if (isset($this->request->uriNodes[5])&&'index'==$this->request->uriNodes[5]) {
      throw new RestException(RestCodes::HTTP_301,
        ...
        array('Location'=>'/'.$this->request->uriNodes[0].'/'
              ...
              .'/'.$this->request->uriNodes[4].'.'.$request->fileExt));
    }
    $mainModule->template=$this->loadSiteTemplate('/'.$node.'/'
        .$this->core->document->type.'/'
        .(isset($this->request->uriNodes[5])?$this->request->uriNodes[5]:'index')
        .'.tpl','mainModules.0');
