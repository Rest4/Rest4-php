Element.implement({
  getPoint: function (hPos, vPos) {
    ...
    switch (hPos) {
    case 'left':
      point.x = 0 + (this.getStyle('border-left-width').toInt() > 0 ?
        this.getStyle('border-left-width').toInt() : 0);
      break;
    case 'center':
      point.x = size.x / 2;
      break;
    case 'right':
      point.x = size.x - (this.getStyle('border-right-width').toInt() > 0 ?
        this.getStyle('border-right-width').toInt() : 0);
      break;
    default:
      point.x = (hPos ? hPos : 0);
      break;
    ...
    switch (vPos) {
    case 'top':
      point.y = 0 + (this.getStyle('border-top-width').toInt() > 0 ?
        this.getStyle('border-top-width').toInt() : 0);
      break;
    case 'center':
      point.y = size.y / 2;
      break;
    case 'bottom':
      point.y = size.y - (this.getStyle('border-bottom-width').toInt() > 0 ?
        this.getStyle('border-bottom-width').toInt() : 0);
      break;
    default:
      point.y = (vPos ? vPos : 0);
      break;
  ...
  hitTest: function (aElement) {
    ...
    if ((ePos.x >= aPos.x + aSize.x) || (ePos.x + eSize.x <= aPos.x)
      || (ePos.y >= aPos.y + aSize.y) || (ePos.y + eSize.y <= aPos.y)) {
      return false;
    } else {
      return true;
    }
  ...
  setAnchoredPosition: function (aElement, params) {
    ...
    var parentPos = (this.offsetParent ? this.offsetParent : document.body)
      .getAbsolutePosition();
    ...
    if (params.noVScroll || params.noHScroll || params.noMoreVScroll
      || params.noMoreHScroll || params.resizeInsteadVScroll
      || params.resizeInsteadHScroll) {
      var docScroll = window.getScroll();
      var docScrollSize = window.getScrollSize();
      var docSize = window.getSize();
      var eSize = this.getSize();
      if (params.resizeInsteadVScroll || params.noVScroll
        || params.noMoreVScroll) {
        ...
        } else if (eSize.y < docSize.y
          && newPos.y + eSize.y > docScroll.y + docSize.y) {
          if (params.noVScroll)
            newPos.y = docScroll.y + docSize.y - eSize.y;
          else if (params.noMoreVScroll && eSize.y < docScrollSize.y
            && newPos.y + eSize.y > docScrollSize.y - docScroll.y) {
            newPos.y = docScrollSize.y - eSize.y;
          }
        }
      }
      if (params.resizeInsteadHScroll || params.noHScroll
        || params.noMoreHScroll) {
        ...
        } else if (eSize.x < docSize.x
          && newPos.x + eSize.x > docScroll.x + docSize.x) {
          if (params.noHScroll) {
            newPos.x = docScroll.x + docSize.x - eSize.x;
          } else if (params.noMoreHScroll && eSize.x < docScrollSize.x
            && newPos.x + eSize.x > docScrollSize.x - docScroll.x) {
