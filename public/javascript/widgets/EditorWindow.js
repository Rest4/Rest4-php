var EditorWindow=new Class({
  ...
  initialize: function(desktop,options) {
    ...
    this.app.registerCommand('win'+this.id+'-new',
      this.newDocument.bind(this));
    this.app.registerCommand('win'+this.id+'-close',
      this.closeDocument.bind(this));
    this.app.registerCommand('win'+this.id+'-open',
      this.openDocument.bind(this));
    this.app.registerCommand('win'+this.id+'-save',
      this.saveDocument.bind(this));
    this.app.registerCommand('win'+this.id+'-saveAs',
      this.saveAsDocument.bind(this));
  },
  // Rendering window
  render : function() {
    ...
    // Menu
    this.options.menu=[{
      'label':this.locale.menu_file,
      'title':this.locale.menu_file_tx,
      'childs':[{
        'label':this.locale.menu_new,
        'command':'new',
        'title':this.locale.menu_new_tx
      },{
        'label':this.locale.menu_open,
        'command':'open',
        'title':this.locale.menu_open_tx
      },{
        'label':this.locale.menu_save,
        'command':'save',
        'title':this.locale.menu_save_tx
      },{
        'label':this.locale.menu_save_as,
        'command':'saveAs',
        'title':this.locale.menu_save_as_tx
      },{
        'label':this.locale.menu_close,
        'command':'close',
        'title':this.locale.menu_close_tx
      }]
    }];
    // Drawing window
    this.parent();
    ...
    var tpl='<form class="vbox"><textarea id="win'+this.id+'-textarea">'
      +this.options.content+'</textarea></form>';
    this.view.innerHTML=tpl;
  },
  loadContent: function() {
    if(this.options.path) {
      var req=new RestRequest({'url':this.options.path,'method':'get'});
  ...
  newDocument: function(event) {
    if(this.temp_content != $('win'+this.id+'-textarea').value) {
      this.app.createWindow('ConfirmWindow',{
        'name':this.locale.confirm_new_title,
        'content':this.locale.confirm_new_content,
        'onValidate':this.confirmNewOk.bind(this),
        'onCancel':this.confirmNewCancel.bind(this)
      });
    } else {
  ...
  openDocument: function(event) {
    if(this.temp_content != $('win'+this.id+'-textarea').value) {
      this.app.createWindow('ConfirmWindow',{
        'name':this.locale.confirm_open_title,
        'content':this.locale.confirm_open_content,
        'onValidate':this.confirmOpenOk.bind(this),
        'onCancel':this.confirmOpenCancel.bind(this)
      });
    } else {
      var index=this.options.path.lastIndexOf('/');
      this.app.createWindow('PromptFileWindow',{
        'onValidate':this.documentOpened.bind(this),
        'mime':'(text\/(.*)|application\/internal)',
        'path':(index>=0?this.options.path.substr(0,index+1):'/')
          .replace('/fs','')
      });
    }
  },
  documentOpened: function(event, output) {
    if(!output) {
      this.app.createWindow('AlertWindow',{
        'name':this.locale.content_error_title_loading,
        'content':this.locale.content_error_loading
      });
    } else {
      this.options.path='/fs'+output;
      this.loadContent();
    }
  },
  // Save document
  ...
  saveDone: function() {
    this.app.createWindow('AlertWindow',{
      'name':this.locale.save_as_done_title,
      'content':this.locale.save_as_done_content
    });
    this.temp_content=$('win'+this.id+'-textarea').value;
  ...
  saveError: function(req) {
    this.app.createWindow('AlertWindow',{
      'name':this.locale.save_as_error_title,
      'content':this.locale.save_as_error_content
    });
  },
  // Save as document
  saveAsDocument: function(event) {
    ...
    this.app.createWindow('PromptFileWindow',{
      'type':'save',
      'path':(this.options.path.substring(0,this.options.path.lastIndexOf('/')+1)||'/'),
      'nameFile':nameFile,
      'onValidate':this.documentAsSaved.bind(this)
    });
  },
  ...
  closeDocument: function(event) {
    if($('win'+this.id+'-textarea')
      &&this.temp_content != $('win'+this.id+'-textarea').value) {
      this.app.createWindow('ConfirmWindow',{
        'name':this.locale.confirm_close_title,
        'content':this.locale.confirm_close_content,
        'onValidate':this.confirmCloseOk.bind(this),
        'onCancel':this.confirmCloseCancel.bind(this)
      });
    } else {
      this.close();
    }
