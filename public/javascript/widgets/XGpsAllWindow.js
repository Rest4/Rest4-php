var XGpsAllWindow=new Class({
	...
	initialize: function(desktop,options) {
		...
		// Registering window
		this.app.registerCommand('win'+this.id+'-refresh',this.loadContent.bind(this));
		this.app.registerCommand('win'+this.id+'-view',this.view.bind(this));
	},
	// Window
	load : function() {
		this.addReq(this.app.getLoadDatasReq('/db/vigisystem/vehicles/list.dat?mode=extend&limit=0&orderby=linked_users_firstname',this.users={}));
		this.parent();
		},
	render : function() {
		this.options.name=this.locale.title;
		// Filling menu
		this.options.menu.push({'label':this.locale.menu_refresh,'command':'refresh','title':this.locale.menu_refresh_tx});
		this.parent();
		// Rendering content
	...
	renderMap: function(req)	{
			...
			for(var i=0, j=this.entries.length; i<j; i++)
				...
				this.entries[i].fields=(this.entries[i].gps?this.entries[i].gps.split(','):[]);
				if(this.entries[i].fields[2]&&this.entries[i].fields[3])
					...
					for(var k=0, l=this.users.entries.length; k<l; k++)
						...
						if(this.users.entries[k]==this.entries[i].fields[1])
							{
							this.entries[i].fields[8]=this.users.entries[k].user_login;
							this.entries[i].fields[9]=this.users.entries[k].user_label;
							}
						}
					latLng=new google.maps.LatLng(this.entries[i].fields[3],this.entries[i].fields[2]);
	...
	showInfo : function ()
		...
		this.infowindow = new google.maps.InfoWindow({
			content: '<h2><a href="#win'+this.window.id+'-view:'+this.entry.login+'">'+this.entry.label+'</a></h2>'
				+'<p><strong>'+this.window.locale.map_date+'</strong> '+this.entry.fields[0]+'<br />'
				...
				+'<strong>'+this.window.locale.map_heading+'</strong> '+this.entry.fields[5]+this.window.locale.map_heading_unit+'<br />'
				+'<strong>'+this.window.locale.map_altitude+'</strong> '+this.entry.fields[6]+this.window.locale.map_altitude_unit+'<br />'
				+'<strong>'+this.window.locale.map_satellite+'</strong> '+this.entry.fields[7]+'</p>'
				});
