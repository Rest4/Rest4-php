var PromptDateWindow=new Class({
	...
	initialize: function(app,options)
		...
		//Default setting
		var date=new Date();
		if((!this.options.multiple)&&this.options.output.dates.length&&this.options.output.dates[0]!='0000-00-00')
			{
	...
	render : function()
		...
		// Creating form
		var tpl='<form id="win'+this.id+'-handleForm">'
			//+'<label>'+this.locale.month+'</label>'
			+'<select name="month" id="win'+this.id+'-month">';
		for(var i=0; i<12; i++)
			tpl+='<option value="'+i+'" '+(this.currentMonth==i?'selected="selected"':'')+'>'+this.locale.months[i]+'</option>';
		tpl+='</select>'
			//+'<label>'+this.locale.year+'</label>'
			+'<input type="number" name="year" value="'+this.currentYear+'" pace="1" min="0" id="win'+this.id+'-year">';
			+'</form>';
		// Filling menu
		this.options.menu[0]={'label':this.locale.menu_previousyear,'command':'prev:year','title':this.locale.menu_previousyear_tx};
		this.options.menu[1]={'label':this.locale.menu_previousmonth,'command':'prev:month','title':this.locale.menu_previousmonth_tx};
		this.options.menu[2]={'tpl':tpl};
		this.options.menu[3]={'label':this.locale.menu_nextmonth,'command':'next:month','title':this.locale.menu_nextmonth_tx};
		this.options.menu[4]={'label':this.locale.menu_nextyear,'command':'next:year','title':this.locale.menu_nextyear_tx};
		// Drawing window
		this.parent();
	},
	renderContent : function()
		...
		var day;
		if(this.currentMonth==0||this.currentMonth==2||this.currentMonth==4||this.currentMonth==6||this.currentMonth==7||this.currentMonth==9||this.currentMonth==11)
			day=31;
		else if(this.currentMonth==3||this.currentMonth==5||this.currentMonth==8||this.currentMonth==10)
			day=30;
		else 
			{
		...
		tpl+='	<form action="#win'+this.id+'-validate">'
			+'		<fieldset><table class="date">'
			+'		<thead><tr>'
			+'			<th>'+this.locale.week+'</th>';
		for(var i=0; i<7; i++)
			{
			tpl+='			<th>'+this.locale.days[(i+parseInt(7-this.locale.day_gap))%7]+'</th>';
			}
		tpl+='		</tr></thead><tbody>';
		...
		for(var i=0,j=0; i<42; i++)
			...
			if(i<gap||i>=(gap+day))
				{
				tpl+='			<td class="disabled"></td>';
				}
			else
				{
				j++;
				var curDate=this.currentYear+'-'+(((this.currentMonth+1)+'').length<2?'0':'')+(this.currentMonth+1)+'-'+((j+'').length<2?'0':'')+j;
				tpl+='			<td'+(this.options.output.dates.indexOf(curDate)>=0?' class="checked"':'')+'>'
	...
	getWeekNumber : function()
		...
		return Math.floor((countDays-((new Date(this.currentYear,0,1).getDay()+parseInt(this.locale.day_gap))%7))/7)+1;
		},
