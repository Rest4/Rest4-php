var WebWindow=new Class({
	...
	initialize: function(app,options) {
		...
		for(var i=this.requiredOptions.length-1; i>=0; i--)
			...
			if(!options[this.requiredOptions[i]])
				{
				this.app.error('Cannot find this required option: '+this.requiredOptions[i]);
				return false;
				}
			}
		...
		this.app.registerCommand('win'+this.id+'-maximize',this.maximize.bind(this));
		// Maps
		if(this.needMaps)
	...
	render: function()
		...
		if(!this.options.noToolbox)
			...
			tpl+='<div class="toolbox">'
				...
				+'		<li class="flex"><h1><a class="mover" href="#win'+this.id+'-move">'+this.options.name+' '+this.id+'</a></h1></li>';
		...
		if(this.options.forms&&this.options.forms.length)
			...
			for(var i=0, j=this.options.forms.length; i<j; i++)
				{
				this.options.menu.push({'label':this.options.forms[i].label,'command':'showForm:'+i,'title':this.options.forms[i].title});
				}
		...
		if(this.locale.help_content)
			{
			this.options.menu[this.options.menu.length]={'label':this.locales['WebWindow'].help_link,'command':'help','title':this.locales['WebWindow'].help_link_tx};
		...
		if(this.options.menu&&this.options.menu.length)
			...
			for(var i=0, j=this.options.menu.length; i<j; i++)
				...
				if(this.options.menu[i].tpl)
					...
					tpl+='	<a href="#'+(this.options.menu[i].command?'win'+this.id+'-'+this.options.menu[i].command:'showMenu')+'" title="'+this.options.menu[i].title+'" class="button">'+this.options.menu[i].label+'</a>';
				if(this.options.menu[i].childs&&this.options.menu[i].childs.length)
					...
					for(var k=0, l=this.options.menu[i].childs.length; k<l; k++)
						...
						tpl+='		<li class="menu right">'
							+'			<a href="#'+(this.options.menu[i].childs[k].command?'win'+this.id+'-'+this.options.menu[i].childs[k].command:'showMenu')+'" title="'+this.options.menu[i].childs[k].title+'" class="button">'+this.options.menu[i].childs[k].label+'</a>';
	...
	update: function()
		...
		if(this.noticeElement)
			this.noticeElement.setAnchoredPosition(this.view,{aHPos:'center',aVPos:15,hPos:'center',vPos:'top'});
		},
	move: function(point)
		{
	...
	notice: function(message,options)
		...
		this.noticeElement.setAnchoredPosition(this.view,{aHPos:'center',aVPos:15,hPos:'center',vPos:'top'});
		this.hideNoticeDelay=this.hideNotice.bind(this).delay((options&&options.delay?options.delay:3000))
		},
	hideNotice: function()
		{
	...
	loadError: function(event)
		{
		if(this.view)
			this.view.innerHTML='<div class="box">'+this.locales['WebWindow'].load_error+'</div>';
		else
	...
	showHelp: function(event)
		{
		this.app.createWindow('AlertWindow',{'synchronize':false,'name':this.locales['WebWindow'].help_name,'content':this.locale.help_content});
		},
	...
	syncWindows: function(syncFunction,syncTests)
		...
		for(var i=this.app.windows.length-1; i>=0; i--)
			{
			if(this.app.windows[i]!=this&&this.app.windows[i].classNames[0]==this.classNames[0])
				{
				var syncIt=true;
				if(syncTests)
					{
