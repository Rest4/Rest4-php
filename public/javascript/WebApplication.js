var WebApplication=new Class({
  ...
  loaded: function() {
    if(this.locales['WebApplication']&&this.user&&this.user.groupName) {
      ...
      if(!window[this.user.groupName[0].toUpperCase()
        +this.user.groupName.substring(1)+'Profile']) {
        this.loadScript('/mpfs/public/javascript/profiles/'
          +this.user.groupName[0].toUpperCase()
          +this.user.groupName.substring(1)+'Profile.js');
      }
      // Running profile script
      if(window[this.user.groupName[0].toUpperCase()
        +this.user.groupName.substring(1)+'Profile']) {
        this.profile=new window[this.user.groupName[0].toUpperCase()
          +this.user.groupName.substring(1)+'Profile'](this);
      } else {
        this.start();
        this.createWindow('AlertWindow',{
          'content':this.locales['WebApplication']['profile_error']
        });
      }
    }
  },
  ...
  renderInterface: function() {
    ...
    if(this.screenType=='small') {
      ...
      var tpl='<div class="loadingbox">'
        ...
        +'    <li class="menu"><a href="#showMenu" class="button">'
          +this.locales['WebApplication'].menu+'</a>'
        +'      <ul class="menupopup small mainmenu"></ul>'
        +'    </li>'
        +'    <li class="menu"><a href="#showMenu" class="button">'
          +this.locales['WebApplication'].info+'</a>'
        +'    <ul class="menupopup small reverse">'
        +'      <li class="menu flex">'
    ...
    if(this.menu.length) {
      ...
      for(var i=0, j=this.menu.length; i<j; i++) {
        tpl+='<li class="menu '+this.menu1rstClass+'">'
          +'<a href="#'+(this.menu[i].command?this.menu[i].command:'showMenu')
          +'" title="'+this.menu[i].title+'" class="button">'
          +this.menu[i].label
          +'</a>';
  ...
  initEvents: function() {
    ...
    if(window.applicationCache) {
      window.applicationCache.addEventListener('updateready',
        this.updateReady.bind(this), false);
    }
  ...
  handleUnload: function(evt) {
    if(evt) {
      evt.returnValue=this.locales['WebApplication'].back_message;
    }
    // For Safari
  ...
  handleShowMenuEvent: function(event) {
    ...
    while(targetElement!=this.root&&targetElement.nodeName!='LI'
      &&!targetElement.hasClass('menu')) {
      targetElement=targetElement.parentNode;
    }
    if(targetElement.nodeName=='LI'&&targetElement.hasClass('menu')
      &&!targetElement.hasClass('selected')) {
      var menus;
      ...
      if(menus[0]&&menus[0].childNodes.length) {
        ...
        if(this.obstrusiveMenuMode) {
          ...
          menus[0].setAnchoredPosition(targetElement,{
            aHPos:'right',
            aVPos:'top',
            hPos:'right',
            vPos:'bottom'
  ...
  handleDrag: function(event) {
    ...
    if(dWindow) {
      ...
      if(targetElement.nodeName=='A'&&targetElement.hasAttribute('href')
        &&targetElement.getAttribute('href').indexOf('#win')===0) {
        ...
        } else if(command.test(/^win([0-9]+)-resize-(h|w|hw)$/)
          &&!this.winbox.hasClass('mono')) {
  ...
  handleSubmit: function(event) {
    ...
    while(targetElement&&targetElement!=this.root
      &&targetElement.nodeName!='FORM') {
      targetElement=targetElement.parentNode;
    }
    if(targetElement.nodeName=='FORM'
      &&targetElement.hasAttribute('action')
      &&targetElement.getAttribute('action').indexOf('#')===0) {
      event.stop();
  ...
  handleCommand: function(event) {
    ...
    if(targetElement&&targetElement.nodeName=='INPUT'
      ...
      &&targetElement.hasAttribute('formaction')
      &&targetElement.getAttribute('formaction').indexOf('#')===0) {
  ...
  createRestRequest: function(params,revived) {
    if(params.path&&'/' !== params.path[0]) {
      console.log('A RestRequest path should always begin by a slash ('
        + params.path + '), adding automatically, but you should fix it.');
