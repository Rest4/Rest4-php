...
class RestDriver
	...
	function getResponse()
		...
		if(!(isset($this::$drvInf->methods,$this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))})))
			throw new RestException(RestCodes::HTTP_501,'The used method has not been documented yet so it is disabled ('.strtolower(RestMethods::getStringFromMethod($this->request->method)).', '.get_class($this).').');
		if(!method_exists($this,strtolower(RestMethods::getStringFromMethod($this->request->method))))
				throw new RestException(RestCodes::HTTP_405,'The requested method is documented but not implemented by this ressource ('.RestMethods::getStringFromMethod($this->request->method).')');
		if(isset($this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))}->queryParams))
			{
			$this->queryParams=$this->getQueryStringParams();
			}
		else if($this->request->queryString)
			throw new RestException(RestCodes::HTTP_400,'This driver does not define any queryString parameters for this method ('.strtolower(RestMethods::getStringFromMethod($this->request->method)).', '.get_class($this).').');
		// Checking requested content type
		if(!isset($this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))},$this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))}->outputMimes))
			throw new RestException(RestCodes::HTTP_500,'The output mimes for this method lacks in this driver ('.strtolower(RestMethods::getStringFromMethod($this->request->method)).').');
		if($this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))}->outputMimes!='*')
			...
			if($this->request->fileExt)
				...
				if(!$acceptedMime)
					throw new RestException(RestCodes::HTTP_406,'This REST driver don\'t support your request Accept prerogatives for the given method (given: '.$this->request->getHeader('Accept').', can serve: '.$this::$drvInf->methods->{strtolower(RestMethods::getStringFromMethod($this->request->method))}->outputMimes.').');
				else // Could be 300 ?
					throw new RestException(RestCodes::HTTP_301,'Redirecting to the the found ressource corresponding to your Accept prerogative.', '', array('Location'=>RestServer::Instance()->server->location.$this->request->controller.$this->request->filePath.$this->request->fileName.'.'.xcUtils::getExtFromMime($acceptedMime).($this->request->queryString?'?'.$this->request->queryString:'')));
				}
			}
		...
		if($response->getHeader('Content-Type')=='application/internal')
			...
			if($this->request->fileExt=='txt')
				...
				throw new RestException(RestCodes::HTTP_406,'Cannot convert datas to the asked content type (given: '.$this->request->fileExt.':'.$mime.', can serve: dat|json|txt ).');
			}
		return $response;
		}
	function options()
	...
	function getQueryStringParams()
		...
		for($i=0, $j=$queryParams->count(); $i<$j; $i++)
			...
			if(isset($queryParams[$i]->required)&&$queryParams[$i]->required&&isset($queryParams[$i]->value))
				throw new RestException(RestCodes::HTTP_500,'This parameter "'.$queryParams[$i]->name.'" cannot be required when it have a default value.');
			// Unrequired single query params must have a default value
			if((!isset($queryParams[$i]->value))&&(!(isset($queryParams[$i]->required)&&$queryParams[$i]->required))&&!(isset($queryParams[$i]->multiple)&&$queryParams[$i]->multiple))
				throw new RestException(RestCodes::HTTP_500,'This parameter "'.$queryParams[$i]->name.'" has no default value.');
			// Parsing values
			for($k; $k<$l; $k++)
				{
				if($this->request->queryValues[$k]->name==$queryParams[$i]->name)
					...
					if(isset($queryParams[$i]->value)&&$queryParams[$i]->value==$this->request->queryValues[$k]->value)
						throw new RestException(RestCodes::HTTP_400,'The given value for the "'.$queryParams[$i]->name.'" parameter is the default value. Remove the parameter to use it\'s default value');
					$value=xcUtilsInput::filterValue($this->request->queryValues[$k]->value,(isset($queryParams[$i]->type)?$queryParams[$i]->type:'text'),(isset($queryParams[$i]->filter)?$queryParams[$i]->filter:'parameter'));
					if((!$value)&&$value===null)
						throw new RestException(RestCodes::HTTP_400,'The given value for the "'.$queryParams[$i]->name.'" is not matching the following type "'.(isset($queryParams[$i]->type)?$queryParams[$i]->type:'text').'" (filter: '.(isset($queryParams[$i]->filter)?$queryParams[$i]->filter:'parameter').')');
					if(isset($queryParams[$i]->multiple)&&$queryParams[$i]->multiple)
						...
						if($values->{$queryParams[$i]->name}->count()>1&&!(isset($queryParams[$i]->orderless)&&$queryParams[$i]->orderless))
							{
							for($m=0, $n=strlen($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-1]); $m<$n; $m++)
								{
								if(isset($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-2][$m]))
									{
									if(ord($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-1][$m])<ord($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-2][$m]))
										{
										throw new RestException(RestCodes::HTTP_400,'The value #'.$values->{$queryParams[$i]->name}->count().' of the parameter "'.$queryParams[$i]->name.'" is not well ordinated at char '.$m.' ('.$values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-1][$m].'<'.$values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-2][$m].').');
										}
									if(ord($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-1][$m])!=ord($values->{$queryParams[$i]->name}[$values->{$queryParams[$i]->name}->count()-2][$m]))
										{
										break;
			...
			if(isset($queryParams[$i]->required)&&$queryParams[$i]->required&&!isset($values->{$queryParams[$i]->name}))
				{
				throw new RestException(RestCodes::HTTP_400,'This parameter "'.$queryParams[$i]->name.'" is required when using this method for this driver.');
				}
			}
		if($k<$l)
			{
			throw new RestException(RestCodes::HTTP_400,'Bad query param ('.$this->request->queryValues[$k]->name.'), check params list and order.');
			}
		return $values;
		}
	}
