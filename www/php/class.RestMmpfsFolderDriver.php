<?php
class RestMmpfsFolderDriver extends RestDriver
	{
	static $drvInf;
			...
			foreach($this->core->server->paths as $path)
				...
				if(file_exists($path.'..'.$this->filePathes[$k]))
					...
					if(!is_dir($path.'..'.$this->filePathes[$k]))
						throw new RestException(RestCodes::HTTP_500,'The given uri seems to not be a folder (mmpfs'.$filePath.')');
					$exists=true;
					}
				}
			}
...
RestMmpfsFolderDriver::$drvInf=RestMmpfsFolderDriver::getDrvInf();